...
class Game:
    ...
    def __init__(self, screen_width, screen_height, map_size, minimap_size):
        ...
        self.mini_cam_x, self.mini_cam_y = 0,0
    ...
    def draw_mini_map(self, display_surface):
        ...
        # Échelle entre la grande carte et la mini-carte
        map_width_half = self.map_size * self.tile_grass.width_half
        map_height_half = self.map_size * self.tile_grass.height_half

        cam_scale_x = self.mini_map_size_x / (self.map_size * self.tile_grass.width_half)
        cam_scale_y = self.mini_map_size_y / (self.map_size * self.tile_grass.height_half)


        #print (cam_scale_x, cam_scale_y)

        # Calcul des positions et dimensions du rectangle représentant la caméra


        # Les dimensions du rectangle doivent être proportionnelles à la taille de l'écran
        rect_width = self.screen_width * cam_scale_x //2
        rect_height = self.screen_height * cam_scale_y

        rect_x = (self.cam_x + self.map_size * self.tile_grass.width_half) * cam_scale_x - (self.screen_width * cam_scale_x) + rect_width//2
        rect_y = (self.cam_y + self.map_size * self.tile_grass.height_half) * cam_scale_y - (self.screen_width * cam_scale_y)

        pygame.draw.rect(mini_map_surface, (255, 0, 0), (rect_x, rect_y, rect_width, rect_height), 1)
        ...
        # Afficher la mini-carte sur l'écran principal
        #display_surface.blit(mini_map_surface, (self.screen_width - self.mini_map_size_x -10, self.screen_height - self.mini_map_size_y - 10))
    ...
    def handle_mini_map_movement(self, keys):
        """Gère le mouvement du rectangle sur la mini-carte."""
        # Déplacer le rectangle de la mini-carte avec une vitesse réduite
        if keys[K_LEFT]:
            self.mini_cam_x = max(self.mini_cam_x - self.mini_scroll_speed, 0)
        if keys[K_RIGHT]:
            self.mini_cam_x = min(self.mini_cam_x + self.mini_scroll_speed, self.mini_map_size_x)
        if keys[K_UP]:
            self.mini_cam_y = max(self.mini_cam_y - self.mini_scroll_speed, 0)
        if keys[K_DOWN]:
            self.mini_cam_y = min(self.mini_cam_y + self.mini_scroll_speed, self.mini_map_size_y)
