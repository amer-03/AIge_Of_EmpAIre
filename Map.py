...
class TileMap:
    ...
    def __init__(self, size, tile_grass, tile_wood, tile_gold,):
        ...
        self.object_layer = np.full((size, size), None)
    ...
    def add_wood_patches(self):
        ...
        num_patches = random.randint(100,200)
        ...
        for _ in range(num_patches):
            ...
            if self.object_layer[start_y][start_x] is None:
                self.object_layer[start_y][start_x] = "W"  # Placer la première tuile de bois
            ...
            while len(wood_tiles) < patch_size:
                ...
                if 0 <= new_x < self.size and 0 <= new_y < self.size:
                    if self.object_layer[new_y][new_x] is None:  # Placer du bois si la case est d'herbe
    ...
    def add_gold_patches(self):
        ...
        for _ in range(num_patches):
            patch_size = 1#random.randint(min_patch_size, max_patch_size)
            start_x = random.randint(0, self.size - 1)
            start_y = random.randint(0, self.size - 1)

            gold_tiles = [(start_x, start_y)]
            if self.object_layer[start_y][start_x] is None:
                self.object_layer[start_y][start_x] = "G"  # Placer la première tuile d'or
            ...
            while len(gold_tiles) < patch_size:
                ...
                if 0 <= new_x < self.size and 0 <= new_y < self.size:
                    if self.object_layer[new_y][new_x] is None:
    ...
    def render(self, display_surface, cam_x, cam_y):
        ...
        tile_width_half = int(self.tile_grass.width_half * self.zoom)
        tile_height_half = int(self.tile_grass.height_half * self.zoom)
        for row in range(self.size):
            for col in range(self.size):
                ...
                tile_resized = pygame.transform.scale(tile.image, 
                             (int(tile.width * self.zoom), int(tile.height * self.zoom)))
                # Affichage de la tuile
                display_surface.blit(tile_resized, (iso_x, iso_y))

                object_type = self.object_layer[row][col]
                if object_type == "W":
                    tile = self.tile_wood
                    iso_y -= tile.height - self.tile_grass.height + 85 # Décalage pour l'arbre
    ...
    def add_special_block(self):
        ...
        self.map_data[center_y][center_x] = "T"  # Placer un bloc spécial au centre comment faire en sorte que les trees sur le grass